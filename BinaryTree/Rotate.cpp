...
#define HEIGHT_RECT 5  // 직사5각형 세로 길이
...
void drawFilledRectangle(float angle) {
    ...
    // 사각형의 모든 픽셀을 채우기
    for (float y = -HEIGHT_RECT / 2.0; y < HEIGHT_RECT / 2; y += 0.25) {
        for (float x = -WIDTH_RECT / 2.0; x < WIDTH_RECT / 2; x += 0.25) {
            // 회전 변환을 적용
...
int main() {
    ...
    float rotationSpeed = 360.0f / (ROTATION_TIME / 100.0f); // 3초 동안 회전할 각도
    int elapsedTime = 0; // 경과 시간 변수

    while (elapsedTime <= ROTATION_TIME) { // 3초 동안만 실행
        drawFilledRectangle(totalRotation); // 현재 각도로 회전된 꽉 찬 직사각형을 그림
        totalRotation += rotationSpeed;     // 각도 증가
        Sleep(100);                         // 0.1초 대기 (애니메이션 효과)
        elapsedTime += 100;                 // 경과 시간 업데이트
    }
